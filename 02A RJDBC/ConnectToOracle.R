...
"
# In the following, use your path instead of /Library/Java/JavaVirtualMachines/jdk1.7.0_45.jdk/Contents/Home
...
possibleError <- tryCatch(
  ...
  jdbcConnection <- dbConnect(jdbcDriver, "jdbc:oracle:thin:@128.83.138.158:1521:orcl", "C##cs347_ara2322", "orcl_ara2322"),
  error=function(e) e
)
if(!inherits(possibleError, "error")){
  emps <- dbGetQuery(jdbcConnection, "select * FROM CC")
  dbDisconnect(jdbcConnection)
}

ggplot(data = emps) + geom_histogram(aes(x = COMPANY_RESPONSE))
